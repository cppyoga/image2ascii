...
// Size of ASCII art
static const int new_width = 120;

// Ascii gradient
// static const std::string ASCII_CHARS = "$@B%8&WM#*oahkbdpqwmZO0QLCJUYXzcvunxrjft/\\|()1{}[]?-_+~<>i!lI;:,\"^`'. ";
...
void image_to_ascii(const std::string &image_data, int image_width) {
  ...
  int new_height = static_cast<int>(static_cast<double>(height) / width * image_width * 0.45);

  for (int i = 0; i < new_height; ++i) {
    for (int j = 0; j < image_width; ++j) {
      int old_i = i * height / new_height;
      int old_j = j * width / image_width;
...
int main(int argc, char **argv) {
  ...
  image_to_ascii(image_data, width);
